...
class DecisionTree(ABC):
    ...
    def __grow(self,node,D,level):       
        ...
        if depth and msamp and n_cls:
            ...
            self.features_used.append([self.features[feature],level])
            #declare child nodes
            left_node  = Node()
            right_node = Node()
    ...
    def __printtree(self,node,indent=" "):
        ...
        if node.leafv is None:
            ...
            #decide to go left or right?
            print("|-",self.features[feature], "<=", split)
            print("%sleft-->" % (indent), end="")
            self.__printtree(node.get_left_node(),indent + indent)
            print("%sright-->" % (indent), end="")
            self.__printtree(node.get_right_node(),indent + indent)
        else:
            #print the leaf value
            print(round(node.leafv,4))
    ...
    def print_tree(self, tree=None, indent=" "):
        ...
        if tree.leafv is not None:
            print(round(tree.leafv,4))

        else:
            (split,feature) = tree.get_params()
            print("|-",feature, "<=", split)
            print("%sleft-->" % (indent), end="")
            self.print_tree(tree.get_left_node(), indent + indent)
            print("%sright-->" % (indent), end="")
            self.print_tree(tree.get_right_node(), indent + indent)
            
            
    #train the tree model
    def fit(self,Xin,Yin, features=None):
    ...
    #extract features from all the list
